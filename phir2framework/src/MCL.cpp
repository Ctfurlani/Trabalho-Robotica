...
void MCL::sampling(int set, const Action &u)
{
    ...
    double V1 = alpha[0]*u.rot1 + alpha[1]*u.trans;
    double V2 = alpha[2]*u.trans + alpha[3]*(u.rot1 + u.rot2);
    double V3 = alpha[0]*u.rot2 + alpha[1]*u.trans;

    std::normal_distribution<double> sampler1(0.0,V1);
    std::normal_distribution<double> sampler2(0.0,V2);
    std::normal_distribution<double> sampler3(0.0,V3);
    ...
    for (int i = 0; i < numParticles; ++i){
        double sample1 = sampler1(*generator);
        double sample2 = sampler2(*generator);
        double sample3 = sampler3(*generator);

        double rot1 = u.rot1 - sample1;
        double trans = u.trans - sample2;
        double rot2 = u.rot2 - sample3;

        particles[set][i].p.x +=  trans*cos(particles[set][i].p.theta + rot1);
        particles[set][i].p.y +=  trans*sin(particles[set][i].p.theta + rot1);
        particles[set][i].p.theta +=  rot1 + rot2;
    }
}
double MCL::probability(float measured, float expected, double var){
    double _measured = (double)measured;
    double _expected = (double)expected;
    double exp = -1.0/2.0 * (pow((_measured-_expected),2.0)/var);
    return (double)(1.0/sqrt(2.0*M_PI*var)) * pow(M_E, exp);
}

void MCL::weighting(int set, const std::vector<float> &z)
{
    /// TODO: faça a pesagem de todas as particulas
    double var = 50.0; // 50, 75 or 100

    if(set==0){
        /// Estratégia complexa
        /// elimine particulas fora do espaco livre
        for (int i = 0; i < numParticles; ++i){
            int x = (int)(particles[set][i].p.x*scale);
            int y = (int)(particles[set][i].p.y*scale);

            if(mapCells[x][y] == FREE){
               particles[set][i].w = 1.0;
            }
            else
                particles[set][i].w = 0.0;
        }
        ...
        for (int i = 0; i < numParticles; ++i){// for each particle
            if (particles[set][i].w){ // that is, in free space
                double p = particles[set][i].w;
                for (int j = 0; j < 181; j+=10){
                    float measured = z[j];
                    float expected = computeExpectedMeasurement(j,particles[set][i].p);
                    p *= probability(measured, expected, var);
                }
                particles[set][i].w = p;
                sum += p;
            }
        ...
        ///
        for (int i = 0; i < numParticles; ++i){
         particles[set][i].w = sum==0 ? 1.0/numParticles : particles[set][i].w/sum;
         //std::cout << "Particle " << i << ", weights: " << particles[set][i].w << "\n";
        }

    }else{
        /// Estratégia simplificada
        //std::cout << "Simple strategy";
        /// elimine particulas fora do espaco livre
        double sum = 0.0;
        for (int i = 0; i < numParticles; ++i){
            int x = (int)(particles[set][i].p.x*scale);
...
void MCL::resampling(int set)
{
    ...
    std::cout << "NextGen size in resampling: " << nextGeneration.size() << "\n";
    ...
    std::uniform_real_distribution<double> samplerU(0.0, 1.0/numParticles);
    /// Para gerar amostras segundo a distribuicao acima, usa-se:
    /// onde *generator é um gerador de numeros aleatorios (definido no construtor da classe)
    double r = samplerU(*generator);
    double c = particles[set][0].w;
    int i = 0;

    for (int j = 1; j <= numParticles; ++j){
        double U = r + 1.0/numParticles*(j - 1);

        while ( U > c){
            ++i;
            c += particles[set][i].w;
        }
        nextGeneration.push_back(particles[set][i]);
    }
    /// Atualiza conjunto de particulas
    std::cout << "NextGen size: " << nextGeneration.size() << "\n";
